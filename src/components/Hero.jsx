...
<h1 className='flex felx-col xl:space-y-8 md:space-y-4 space-y-2 xl:text-6xl md:text-4xl text-3xl md:font-normal font-bolder text-yellow-500'>
  <span className='flex'>
    {
...
<button className='xl:w-[400px] md:w-[300px] w-[270px] bg-gray-200 md:py-1 py-0 md:px-4 px-2 xl:text-2xl md:text-xl text-base text-gray-900 tracking-widest flex justify-between items-center md:mr-auto md:mx-0 mx:auto rounded-r-3xl' 
 ...
 <div className='flex md:gap-6 gap-2 mr-auto'>
{
  socialIcons.map((social,index)=>(
    <a href={social.link} key={index} className='xl:text-3xl md:text-2xl text-yellow-500 hover:text-white transition-colors duration-500 '>
...
<span className='xl:text-xs md:text-[10px] text-[8px] font-bold -tracking-wide text-yellow-500 absolute -top-5 xl:right-22 lg:right-26 md:right-16 right-10 rotate-[3.5deg] animate-bounce '>Looking for new challenges</span>


<div  className={`xl:h-[150px] h-[100px] px-3 xl:test-lg md:text-base text-xs font-light text-gray-200 text-justify tracking-wide overflow-y-auto transform origin-top custom-scrollbar  ${isTextVisible?'scale-y-100':'scale-y-0'} transition-transform duration-300 `}>
